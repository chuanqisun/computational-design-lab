...
{{#if requiredList}}
  ...
  the item has very high intensity of that mood.
{{else}}
  Analyze the provided item and identify 3-5 moods it evokes. For each mood, provide a single English word with first
  letter Capitalized and an arousal level from 1 to 10, where 1 is calm/low energy and 10 is intense/high energy.
{{/if}}
```
